...
def train(true_positives_filepath, false_positives_filepath):
    ...
    features = data[["chrom", "start", "sv_length", "sv_type", "read_depth", "hmm_llh"]]
    labels = data["label"]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tpvcf', type=str, required=True, help='Path to the VCF of true positive SV calls obtained from a benchmarking dataset.')
    parser.add_argument('--fpvcf', type=str, required=True, help='Path to the VCF of false positive SV calls obtained from running the caller on data that is known to be negative for SVs.')
    parser.add_argument('--outdir', type=str, required=True, help='Path to the output directory.')
    args = parser.parse_args()

    # Run the program.
    logging.info('Training the model...')
    run(args.tpvcf, args.fpvcf, args.outdir)
    logging.info('done.')
